...
@section('primary-column')
    <div class="flex flex-col space-y-3">
        <div class="page-box flex flex-col xs:flex-row xs:justify-between xs:pt-4 mb-4">
            @guest
                <div class="flex xs:flex-col xs:flex-1">
                    <div class="px-4 relative xs:px-0">
                        <div class="w-5 h-5 rounded-full absolute text-white bg-blue-light"
                             style="left: 50%; top: 50%; transform: translate(-50%, -50%)">
                        </div>
                        <div class="border-4 border-blue-light h-full"></div>
                    </div>
        ...
        <div class="page-box">
            ...
            <h2 class="text-2xl text-center font-semibold mb-2 py-2 border-t border-b border-white border-opacity-50">
                Order Complete!
            </h2>
            ...
            <div class="flex flex-col lg:flex-row">
                ...
                @if(\Illuminate\Support\Facades\Auth::guest() && $order->user->user_level_id === \Coeliac\Modules\Member\Models\UserLevel::SHOP)
                    <member-register-order-complete-cta name="{{ $order->user->name }}"
                                                        email="{{ $order->user->email }}"></member-register-order-complete-cta>
                @endif
        ...
        <section class="page-box mt-4">
            ...
            <div class="flex flex-col mt-2 sm:flex-row">
                @foreach($latestRecipes as $recipe)
                    <div
                        class="w-full sm:w-1/3 rounded-lg overflow-hidden flex flex-col shadow-md mb-4 bg-gradient-to-br from-blue to-blue-light {{ !$loop->last ? 'sm:mr-3' : '' }}">
                        <div>
        ...
        <section class="page-box mt-4">
            ...
            <div class="flex flex-col mt-2 sm:flex-row">
                @foreach($latestReviews as $review)
                    <div
                        class="w-full sm:w-1/2 rounded-lg overflow-hidden flex flex-col shadow-md mb-4 bg-gradient-to-br from-blue/30 to-blue-light/30 {{ $loop->first ? 'sm:mr-3' : '' }}">
                        <div>
