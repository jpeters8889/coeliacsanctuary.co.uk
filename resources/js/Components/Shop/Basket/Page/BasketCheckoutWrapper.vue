    ...
    <div class="page-box flex flex-col xs:flex-row xs:justify-between xs:pt-4">
      <template v-for="item in sections">
        <template v-if="item.enabled">
          <div
            :key="item.id"
            class="flex xs:flex-col xs:flex-1"
            :class="item.canVisit ? 'cursor-pointer' : ''"
            @click="switchSection(item)"
          >
            <div class="px-4 relative xs:px-0">
              <div
                class="w-5 h-5 rounded-full absolute text-white"
                :class="headerCircleBackground(item)"
                style="left: 50%; top: 50%; transform: translate(-50%, -50%)"
              />
              <div class="border-4 border-blue h-full" />
            </div>
            <div
              class="flex-1 py-3 xs:text-center"
