<template>
  <div
    class="bg-white shadow overflow-hidden flex flex-col md:rounded-md"
    :class="gridClasses"
  >
    <a
    ...
    <div :class="footerClasses">
      <div class="flex flex-col font-semibold transition-all mb-4 lg:mb-0">
          ...
          <ul class="flex flex-wrap text-xs">
            ...
            >
              <a
                ...
                :href="filteredUrl('tags', tag.slug)"
              >
                {{ tag.tag }}
              </a>
            </li>
        ...
        <template v-if="module === 'reviews'">
          <a
            ...
            :href="`/wheretoeat/${item.eatery.county.slug}/${item.eatery.town.slug}`"
          >
            {{ item.eatery.type.name }} in {{ item.eatery.town.town }}, {{ item.eatery.county.county }}
          </a>
          <global-ui-stars
            class="my-2"
            :stars="item.overall_rating"
          />
        </template>
      </div>
      ...
      <ul :class="footerMetaClasses">
        ...
        <li
          v-if="module !== 'collection'"
          v-text="commentsText(item.comments_count)"
        />
        <li
          v-else
          v-text="collectionItemsText(item.items_count)"
        />
      </ul>
    </div>
  </div>
