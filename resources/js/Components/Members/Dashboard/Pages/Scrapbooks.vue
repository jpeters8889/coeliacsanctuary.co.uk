<template>
  <div>
    ...
    <div
      v-else
      class="mt-2 flex flex-col space-y-2 sm:flex-wrap sm:flex-row justify-between sm:-mx-1"
    >
      <div class="bg-gradient-to-br from-blue/30 to-blue-light/30 rounded p-2 flex flex-col w-full sm:mx-1">
        <strong
          class="block font-semibold text-lg flex items-center cursor-pointer hover:underline"
        ...
        >
          <form-input
            ...
            :on-enter="addScrapbook"
          />
          <a
            class="ml-2 flex items-center leading-none bg-yellow text-sm rounded py-1 px-3 cursor-pointer hover:bg-yellow-light transition-all"
            @click="addScrapbook()"
          >
            Add
          </a>
      ...
      <div
        ...
        :key="scrapbook.id"
        class="sm:w-1/2 sm:px-1"
        @mouseenter="isHoveringOn = scrapbook.id"
        @mouseleave="isHoveringOn = null"
      >
        <div
          class="bg-gradient-to-br from-blue/30 to-blue-light/30 rounded p-2 flex cursor-pointer"
          @click.stop.exact="viewScrapbook = scrapbook"
        >
          <div class="flex-1 flex flex-col">
            <strong class="font-semibold text-lg flex items-center">
              <div
                v-if="scrapbook.id === isEditing"
                class="flex"
              >
                <form-input
                  small
              ...
              <template v-else>
                ...
                <span
                  ...
                  class="text-grey-off hover:text-grey-dark cursor-pointer text-base pl-2"
                >
                  <font-awesome-icon
                    :icon="['fas', 'pen']"
                    @click.stop.exact="editTitle(scrapbook)"
                  />
                </span>
              </template>
            </strong>
          ...
          <div
            v-if="scrapbooks.length > 1"
            v-show="isHoveringOn === scrapbook.id"
            class="text-2xl text-right text-grey-off hover:text-grey-dark cursor-pointer transition-all"
          >
            <font-awesome-icon
              :icon="['far', 'trash-alt']"
    ...
    >
      <modal
        ...
        name="view-scrapbook"
        :title="viewScrapbook.name"
      >
        <div class="w-full min-h-map p-3">
          <loader
            v-if="scrapbookItems.length === 0"
            :show="true"
          />
          ...
          >
            <div
              v-for="item in scrapbookItems"
              :key="item.id"
              class="flex flex-col bg-gradient-to-br from-blue/50 to-blue-light/50 rounded-lg md:flex-row"
            >
              <a
                :href="item.item.link"
                target="_blank"
                class="md:w-1/3 md:p-1"
              >
                <img
                  :src="item.item.image"
                  :alt="item.item.title"
                  class="rounded-t-lg md:rounded-lg"
                >
              </a>

              <div class="p-2 flex-1 md:p-1 flex flex-col">
                <a
                  :href="item.item.link"
                  target="_blank"
                >
                  <h2 class="text-lg font-semibold hover:text-blue-dark transition-all md:leading-none">
                ...
                <div class="mt-2 flex justify-between text-sm">
                  ...
                  <a
                    class="font-semibold hover:text-blue-dark transition-all cursor-pointer"
    ...
    >
      <modal
        small
        name="delete-scrapbook"
        title="Are you sure you want to delete this scrapbook? Any items saved will be lost."
      >
        <div class="flex space-x-4 justify-center mt-2 p-3">
          <a
