...
class ShopOrderCompleteEmailImport extends Import
{
    public function handle()
    {
        $seeds = $this->campaignDatabase
            ->table('sent_transactional')
            ->where('template', 'orderConfirmation')
            ->get();
        ...
        foreach ($seeds as $seed) {
            ...
            NotificationEmail::query()->create([
                'key' => $seed->token,
                'user_id' => $seed->toEmail !== 'contact@coeliacsanctuary.co.uk' ? $order->user->id : admin_user()->id,
