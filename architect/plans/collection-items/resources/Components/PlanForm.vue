    ...
    <div>
        <div class="bg-blue-100 rounded-lg p-4">
            <div class="text-right mb-4">
                <a class="bg-blue-700 py-2 px-4 text-lg leading-none font-semibold cursor-pointer slider-bg inline-block rounded text-white"
            ...
            <table class="w-full leading-none">
                <tr class="bg-blue-900 text-white text-semibold text-left">
                    <th class="p-2 border-r border-blue-200"></th>
                    <th class="p-2 border-r border-blue-200">Type</th>
                    <th class="p-2 border-r border-blue-200">Title</th>
                    <th class="p-2"></th>
                </tr>
                <tr v-for="(item, index) in items" class="border-b border-blue-300 bg-white">
                    <td class="p-2 border-r border-blue-200 text-blue-900" style="width: 50px">
                        <template v-if="items.length > 1">
                            <font-awesome-icon class="cursor-pointer" :icon="['fas', 'caret-up']" v-if="index > 0"
                                               @click="moveItemUp(index)"></font-awesome-icon>
                            <span v-if="index > 0 && index < (items.length - 1)">/</span>
                            <font-awesome-icon class="cursor-pointer" :icon="['fas', 'caret-down']"
                                               v-if="index < (items.length - 1)"
                                               @click="moveItemDown(index)"></font-awesome-icon>
                        </template>
                    </td>
                    ...
                    <td class="text-center py-1" style="width: 65px;">
                        <a class="bg-red-500 p-2 inline-block rounded text-sm leading-none font-semibold cursor-pointer slider-bg"
                    ...
                    <div class="w-full py-3 flex-col">
                        <div v-if="!addModal.selectedItem">
                            <div class="flex w-full items-center mb-2">
                                ...
                                <select class="form-control form-control-input w-full flex-1"
                                        v-model="addModal.searchType">
                                    <option value="blogs">Blogs</option>
                                    <option value="recipes">Recipes</option>
                                    <option value="reviews">Reviews</option>
                                    <option value="wte">Where to Eat</option>
                            ...
                            <div class="flex w-full items-center">
                                ...
                                <div class="relative flex-1">
                                    <input type="text" class="form-control form-control-input w-full"
                                           :class="addModal.searchResults.length ? 'rounded-b-none' : ''"
                                           v-model="addModal.searchString"
                                           @keyup="modalSearch()"/>

                                    <div v-if="addModal.searchResults.length"
                        ...
                        <div v-else>
                            ...
                            <div class="flex w-full items-center mt-2">
                                <textarea :disabled="!addModal.selectedItem"
                                          class="form-control form-control-input w-full flex-1" rows="5"
                                          :class="!addModal.selectedItem ? 'bg-gray-100 cursor-not-allowed' : ''"
                                          v-model="addModal.searchDescription"></textarea>
                            </div>

                            <div class="w-full pt-3 flex justify-center">
                                <button class="button-primary button px-4 py-3 rounded text-xl"
