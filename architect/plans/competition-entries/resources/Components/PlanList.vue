<template>
    <div>
        <div class="flex justify-center">
            <a class="inline-block bg-blue-500 font-semibold text-white py-2 px-8 leading-none mx-auto rounded-lg slider-bg"
               @click="viewEntries">
                {{ value }}
            </a>
        </div>
                ...
                <div class="flex flex-col bg-gray-200 p-2">
                    <div class="flex flex-wrap space-x-2 space-y-2 justify-center items-center">
                        <div class="bg-white rounded-lg p-2 flex flex-col justify-center items-center">
                            <h2 class="text-lg text-blue-700 font-semibold">Total Entries</h2>
                            <span>{{ totalEntries }}</span>
                        </div>

                        <template v-if="hasAdditionalEntries">
                            <div class="bg-white rounded-lg p-2 flex flex-col justify-center items-center">
                                <h2 class="text-lg text-blue-700 font-semibold">Primary Entries</h2>
                                <span>{{ primary }}</span>
                            </div>

                            <div class="bg-white rounded-lg p-2 flex flex-col justify-center items-center"
                                 v-if="facebookLike">
                                <h2 class="text-lg text-blue-700 font-semibold">Facebook Like</h2>
                                <span>{{ facebookLike }}</span>
                            </div>

                            <div class="bg-white rounded-lg p-2 flex flex-col justify-center items-center"
                                 v-if="facebookShare">
                                <h2 class="text-lg text-blue-700 font-semibold">Facebook Share</h2>
                                <span>{{ facebookShare }}</span>
                            </div>

                            <div class="bg-white rounded-lg p-2 flex flex-col justify-center items-center"
                                 v-if="twitterTweet">
                                <h2 class="text-lg text-blue-700 font-semibold">Twitter Tweet</h2>
                                <span>{{ twitterTweet }}</span>
                            </div>

                            <div class="bg-white rounded-lg p-2 flex flex-col justify-center items-center"
                                 v-if="twitterFollow">
                                <h2 class="text-lg text-blue-700 font-semibold">Twitter Follow</h2>
                                <span>{{ twitterFollow }}</span>
                            </div>

                            <div class="bg-white rounded-lg p-2 flex flex-col justify-center items-center"
                                 v-if="shopPurchase">
                                <h2 class="text-lg text-blue-700 font-semibold">Shop Purchase</h2>
                    ...
                    <div class="mt-2">
                        <div class="my-2">
                            <competition-paginator :current="currentPage" :last-page="numberOfPages" :can-go-back="currentPage > 1"
                                        :can-go-forward="currentPage < numberOfPages"></competition-paginator>
                        </div>
                        ...
                        <table class="w-full rounded-xl overflow-hidden">
                            ...
                            <tr class="border-b bg-white border-gray-200" v-for="entry in entries" :data-id="entry.id">
